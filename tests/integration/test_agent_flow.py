...
async def test_complaint_escalation_flow(monkeypatch):
    ...
    state = make_initial_state(
        "Tôi muốn khiếu nại về đơn hàng", "test-complaint", customer_id="cust-complaint"
    )
...
async def test_low_confidence_fallback(monkeypatch):
    ...
    state = make_initial_state("Sản phẩm gì tốt nhất?", "test-low-conf", customer_id="cust-low-conf")
...
async def test_layer1_declined_propagation(monkeypatch):
    ...
    state = make_initial_state("Random unknown query", "test-layer1", customer_id="cust-layer1")
...
async def test_streaming_events_have_required_fields(monkeypatch):
    ...
    with patch("services.ai.ai_router.acompletion", mock_llm):
        async for event in astream_agent(
            "Xin chào!", "stream-test-2", "cust-stream-2", checkpointer=MemorySaver()
        ):
            events.append(event)

...
- **FR-014**: When admin edits state via `request_edit`, a synthetic system message MUST be appended to conversation_history and positioned immediately after the customer's last message (not at tail). Structure: `{type: "system_override", timestamp, field, old_value, new_value, pause_id}`. This allows LLM to see the correction in temporal context.
...
- **FR-020**: On graph resume from "stale_data" re-pause (FR-026), append a synthetic "SYSTEM [Stale Data]: price updated from [old] to [new]" message positioned immediately after the customer's last message, using the same structure as FR-014.
...
- **FR-032**: escalation_count is a cumulative lifetime counter incremented on each HITL pause. It MUST NOT increment for "stale_data_price_change" pauses (FR-026). It MUST NEVER decrement or reset. Check escalation_count BEFORE calling interrupt(): if escalation_count ≥ 2 on next pause, bypass interrupt() and route directly to rejection/customer_support_node (implementing the 2-pause maximum). escalation_count values: 0–1 allowed (2 pauses); escalation_count ≥ 2 triggers force rejection.
...
- **SC-014**: Maximum 2 HITL pauses per order (escalation_count values 0–1 allowed; 3rd escalation trigger checks escalation_count ≥ 2 and bypasses pause to force rejection/support escalation); zero infinite HITL loops

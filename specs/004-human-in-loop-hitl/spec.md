...
- **FR-022**: When HITLMetadata status transitions to "escalated", all unprocessed QueuedMessages for the session MUST be included in the SupportQueue.context_snapshot. New messages received after escalation MUST continue to be queued (Gateway behavior unchanged).
- **FR-023**: Intent classification in `queue_consumer_node` MUST use the cheap/economy model (not premium). The classifier receives ALL queued messages as a single batch (not individually) with recency weighting. If classifier confidence < 0.6 on the net intent of the batch, the system MUST default conservatively to CONFIRM/FOLLOW_UP.
...
- **FR-025**: `queue_consumer_node` MUST perform an **orphaned tool call scan** as its absolute first step, before any intent classification or routing. For each AIMessage in conversation history with tool_calls that have no corresponding ToolMessage, inject a synthetic ToolMessage: `{tool_call_id: X, content: "Operation status reset: customer request changed during admin review pause."}`. This MUST run even if there are zero queued messages to process.
...
- **FR-029**: `queue_consumer_node` MUST classify ALL queued messages as a single batch (send entire message list to intent classifier in one call). Individual message-by-message classification is prohibited. The batch classifier weights messages by recency — the final message in temporal order carries the most weight for net intent determination.
- **FR-030**: All QueuedMessage state mutations within `queue_consumer_node` (marking processed=True, acknowledged_by_admin consumption, routing decision record) MUST be committed in a single atomic database transaction. If the transaction fails or the node crashes, all messages MUST revert to their pre-processing state so the entire batch can be safely retried.

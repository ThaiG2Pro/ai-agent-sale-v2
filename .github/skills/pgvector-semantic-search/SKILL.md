...
description: |
  Vector storage/search: pgvector extension, halfvec columns, HNSW indexes, cosine distance queries.
  Triggers: pgvector, embedding, vector, halfvec, HNSW, cosine distance, <=> operator, semantic search.
  Scope: Vector DB setup only - NOT for generating embeddings or LLM calls (see async-python-patterns).
---

...
name: fastapi
description: |
  FastAPI framework: routers, Pydantic schemas, dependency injection, middleware, SQLAlchemy async integration.
  Triggers: @router, endpoint, Depends(), Pydantic, BaseModel, 422 error, CORS, lifespan, APIRouter.
  Scope: API layer only - NOT for async patterns (see async-python-patterns) or agents (see langgraph).
user-invocable: true
---

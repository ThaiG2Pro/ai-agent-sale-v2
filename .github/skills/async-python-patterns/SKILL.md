...
description: |
  Async/await patterns for non-blocking I/O: database queries, LLM calls, concurrent requests.
  Triggers: async def, await, asyncio.gather, Semaphore, event loop, blocking, concurrent, rate limiting.
  Scope: Concurrency engine - NOT for FastAPI setup or LangGraph state (see those skills instead).
---
